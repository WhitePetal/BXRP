...
namespace BXRenderPipeline.LightTransport
{
    ...
    public struct BufferID : IEquatable<BufferID>
    {
        ...
        public BufferSlice<T> Slice<T>(UInt64 offset = 0) where T : struct
        {
        ...
        {
            return obj is BufferID other && Equals(other);
        }
    ...
    public struct BufferSlice<T> : IEquatable<BufferSlice<T>> where T : struct
    {
        ...
        public BufferSlice<U> SafeReinterpret<U>() where U : struct
        {
            if (!UnsafeUtility.IsBlittable<U>())
                throw new ArgumentException($"Type {typeof(U)} must be blittable.");
        ...
        public unsafe BufferSlice<U> UnsafeReinterpret<U>() where U : struct
        {
            int oldSize = UnsafeUtility.SizeOf<T>();
            int newSize = UnsafeUtility.SizeOf<U>();
        ...
        public override int GetHashCode()
        {
            return HashCode.Combine(Id, Offset);
        }

        public bool Equals(BufferSlice<T> other)
        {
            return other.Id == Id && other.Offset == Offset;
        }

        public override bool Equals(object obj)
        {
            return obj is BufferSlice<T> other && Equals(other);
        }

        public static bool operator ==(BufferSlice<T> a, BufferSlice<T> b)
        {
            return a.Equals(b);
        }
        public static bool operator !=(BufferSlice<T> a, BufferSlice<T> b)
    ...
    public class ReferenceContext : IDeviceContext
    {
        ...
        public void Dispose()
        {
            ...
            {
                UnityEngine.Debug.Assert(entry.Value.IsCreated, "A buffer was unexceptedly not created.");
                entry.Value.Dispose();
            }
        }
        ...
        {
            UnityEngine.Debug.Assert(count != 0, "Buffer element count cannot be zero.");
            UnityEngine.Debug.Assert(stride != 0, "Buffer stride cannot be zero.");
        ...
        {
            UnityEngine.Debug.Assert(buffers.ContainsKey(id), "Invalid buffer ID given.");
            buffers[id].Dispose();
            buffers.Remove(id);
        }
        ...
        {
            UnityEngine.Debug.Assert(buffers.ContainsKey(dst.Id), "Invalid buffer ID given.");
        ...
        public void ReadBuffer<T>(BufferSlice<T> src, NativeArray<T> dst) where T : struct
        {
            UnityEngine.Debug.Assert(buffers.ContainsKey(src.Id), "Invalid buffer ID given");
        ...
        public void ReadBuffer<T>(BufferSlice<T> src, NativeArray<T> dst, EventID id) where T : struct
        {
            ReadBuffer(src, dst);
        }
