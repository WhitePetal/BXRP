...
{
    public struct BXShaderBitArray
    {
        private const int k_BitsPerElement = 32;
        private const int k_ElementShift = 5;
        private const int k_ElementMask = (1 << k_ElementShift) - 1;
		...
		{
            if (bitCapacity > bitCount) return;

            int newElemCount = ((bitCount + (k_BitsPerElement - 1)) / k_BitsPerElement);
            if (newElemCount == m_Data?.Length) return;
        ...
        public bool this[int index]
		{
			get
			{
                GetElementIndexAndBitOffset(index, out int elemIndex, out int bitOffset);
				unsafe
				{
