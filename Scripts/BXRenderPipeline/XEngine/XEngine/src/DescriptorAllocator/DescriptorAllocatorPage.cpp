	...
	, m_NumDescriptorsInHeap(numDescriptors)
{
	...
	ThrowIfFaild(device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_DescriptorHeap)));
...
DescriptorAllocation DescriptorAllocatorPage::Allocate(uint32_t numDescriptors)
{
	...
	// Compute the new free block that reults from splitting this block
...
{
	// Find the first element whose offset is greather than the specified offset.
