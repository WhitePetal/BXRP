...
ComPtr<ID3D12GraphicsCommandList4> CommandQueue::GetCommandList()
{
	...
	if (!m_CommandAllocatorQueue.empty() && IsFenceComplete(m_CommandAllocatorQueue.front().fenceValue))
	{
		...
		ThrowIfFaild(commandAllocator->Reset());
	}
	else
	{
	...
	if (!m_CommandListQueue.empty())
	{
		...
		ThrowIfFaild(commandList->Reset(commandAllocator.Get(), nullptr));
	}
	else
	{
	...
	// retrieved when the command list it executed.
	ThrowIfFaild(commandList->SetPrivateDataInterface(__uuidof(ID3D12CommandAllocator), commandAllocator.Get()));
...
uint64_t CommandQueue::ExecuteCommandList(ComPtr<ID3D12GraphicsCommandList4> commandList)
{
	...
	ThrowIfFaild(commandList->GetPrivateData(__uuidof(ID3D12CommandAllocator), &dataSize, &commandAllocator));
	...
	m_CommandAllocatorQueue.push(CommandAllocatorEntry{
		fenceValue,
		commandAllocator
		});
	m_CommandListQueue.push(commandList);

	// The ownership of the command allocator has been transfered to the Comptr
