...
namespace BXRenderPipeline
{
    struct BuddyAllocation
    {
        ...
        public uint2 index2D => BXSpaceFillingCurves.DecodeMorton2D((uint)index);
    }

    unsafe struct BXBuddyAllocator : IDisposable
    {
        struct Header
        {
        ...
        // The offsets for the arrays are stored together with the pointer to avoid a double look-up when accessing data.
        void* m_Data;
        ...
        public BXBuddyAllocator(int levelCount, int branchingOrder, Allocator allocator = Allocator.Persistent)
        {
        ...
        public bool TryAllocate(int requestedLevel, out BuddyAllocation allocation)
        {
            ...
            int dataIndex;
            {
                ...
                Assert.AreNotEqual(freeMask, 0ul);
        ...
        public void Free(BuddyAllocation allocation)
        {
            ...
            while(level >= 0)
            {
                ...
                var supoerBlockMask = ((1ul << Pow2(header.branchingOrder)) - 1) << ((bitIndex >> header.branchingOrder) * Pow2(header.branchingOrder));
                ...
                if(level == 0 || (~freeMask & supoerBlockMask) != 0)
                {
                    freeMasks[maskIndex] = freeMask;
                    if (wasZero)
                    {
