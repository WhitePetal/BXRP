...
namespace BXGeometryGraph
{
        ...
        IGeneratesShaderFunction
    {
        ...
        protected class SlotAttribute : Attribute
        {
            ...
            public GeometryStageCapability stageCapability { get; private set; }

            public SlotAttribute(int mSlotId, Binding mImplicitBinding, GeometryStageCapability mStageCapability = GeometryStageCapability.All)
            {
                slotId = mSlotId;
            ...
            public SlotAttribute(int mSlotId, Binding mImplicitBinding, bool mHidden, GeometryStageCapability mStageCapability = GeometryStageCapability.All)
            {
                slotId = mSlotId;
                binding = mImplicitBinding;
                hidden = mHidden;
            ...
            public SlotAttribute(int mSlotId, Binding mImplicitBinding, float defaultX, float defaultY, float defaultZ, float defaultW, GeometryStageCapability mStageCapability = GeometryStageCapability.All)
            {
                slotId = mSlotId;
                binding = mImplicitBinding;
        ...
        public sealed override void UpdateNodeAfterDeserialization()
        {
            ...
            foreach (var par in method.GetParameters())
            {
                ...
                if (attribute.binding == Binding.None && !par.IsOut && par.ParameterType == typeof(Color))
                    s = new ColorGeometrySlot(attribute.slotId, name, par.Name, SlotType.Input, attribute.defaultValue ?? Vector4.zero, ColorMode.Default, stageCapability: attribute.stageCapability, hidden: attribute.hidden);
                ...
                {
                    Debug.Log("convertToSlotValueType: " + ConvertTypeToSlotValueType(par));
                    s = GeometrySlot.CreateGeometrySlot(
                        ...
                        geometryStageCapability: attribute.stageCapability,
        ...
        private static GeometrySlot CreateBoundSlot(Binding attributeBinding, int slotId, string displayName, string shaderOutputName, GeometryStageCapability geometryStageCapability, bool hidden = false)
        {
        ...
        public void GenerateNodeShaderCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            using (var tempSlots = PooledList<GeometrySlot>.Get())
            {
                ...
                {
                    sb.AppendLine(outSlot.concreteValueType.ToGeometryString(PrecisionUtil.Token) + " " + GetVariableNameForSlot(outSlot.id) + ";");
                }
        ...
        private string GetFunctionName()
        {
            ...
            return function.Name + (function.IsStatic ? string.Empty : "_" + objectId) + "_$precision"
                + (this.GetSlots<DynamicVectorGeometrySlot>().Select(s => NodeUtils.GetSlotDimension(s.concreteValueType)).FirstOrDefault() ?? "")
                + (this.GetSlots<DynamicVectorGeometrySlot>().Select(s => NodeUtils.GetSlotDimension(s.concreteValueType)).FirstOrDefault() ?? "");
        }
        ...
        private string GetFunctionHeader()
        {
            ...
            using (var tempSlots = PooledList<GeometrySlot>.Get())
            {
                ...
                foreach (var slot in tempSlots)
                {
                    ...
                    header += slot.concreteValueType.ToGeometryString(PrecisionUtil.Token) + " " + slot.geometryOutputName;
                }
