...
namespace BXGeometryGraph
{
    ...
    abstract class AbstractGeometryNode : JsonObject, IGroupItem, IRectInterface
    {
        ...
        public virtual string documentationURL => documentationPage;
        ...
        public void GetInputSlots<T>(List<T> foundSlots) where T : GeometrySlot
        {
        ...
        public virtual void GetInputSlots<T>(GeometrySlot startingSlot, List<T> foundSlots) where T : GeometrySlot
        {
            GetInputSlots(foundSlots);
        }

        public void GetOutputSlots<T>(List<T> foundSlots) where T : GeometrySlot
        {
            foreach (var slot in m_Slots.SelectValue())
            {
        ...
        public virtual void GetOutputSlots<T>(GeometrySlot startingSlot, List<T> foundSlots) where T : GeometrySlot
        {
            GetOutputSlots(foundSlots);
        }

        public void GetSlots<T>(List<T> foundSlots) where T : GeometrySlot
        {
            foreach (var slot in m_Slots.SelectValue())
            {
        ...
        public string GetSlotValue(int inputSlotId, GenerationMode generationMode, ConcretePrecision concretePrecision)
        {
            ...
            return slotValue.Replace(PrecisionUtil.Token, concretePrecision.ToGeometryString());
        }
        ...
        public AbstractGeometryProperty GetSlotProperty(int inputSlotId)
        {
            if (owner == null)
                return null;
...
//#endif // PROCEDURAL_VT_IN_GRAPH
        ...
        {
            var slot = FindOutputSlot<GeometrySlot>(fromSocketRef.slotId);
            if (slot == null)
                return string.Empty;
        ...
        public AbstractGeometryNode GetInputNodeFromSlot(int inputSlotId)
        {
            var inputSlot = FindSlot<GeometrySlot>(inputSlotId);
            if (inputSlot == null)
                return null;
            ...
            AbstractGeometryNode fromNode = null;
            if (edges.Count() > 0)
            {
        ...
        public virtual void UpdatePrecision(List<GeometrySlot> inputSlots)
        {
        ...
        public virtual void Concretize()
        {
            ...
            using (var inputSlots = PooledList<GeometrySlot>.Get())
            using (var outputSlots = PooledList<GeometrySlot>.Get())
            {
        ...
        {
            foreach (var slot in this.GetInputSlots<GeometrySlot>())
            {
                if (slot.isConnected)
                {
                    var edge = owner.GetEdges(slot.slotReference).First();
                    var outputNode = edge.outputSlot.node;
                    var outputSlot = outputNode.GetOutputSlots<GeometrySlot>().First(s => s.id == edge.outputSlot.slotId);
        ...
        public virtual string GetVariableNameForSlot(int slotId)
        {
            ...
            return string.Format("_{0}_{1}_{2}_{3}", GetVariableNameForNode(), NodeUtils.GetHLSLSafeName(slot.geometryOutputName), unchecked((uint)slotId), slot.concreteValueType.ToPropertyType().ToString());
        }
        ...
        public GeometrySlot AddSlot(GeometrySlot slot, bool attemptToModifyExistingInstance = true)
        {
            if (slot == null)
            {
                throw new ArgumentException($"Trying to add null slot to node {this}");
            }
            GeometrySlot foundSlot = FindSlot<GeometrySlot>(slot.id);
        ...
        public T FindSlot<T>(int slotId) where T : GeometrySlot
        {
            foreach (var slot in m_Slots.SelectValue())
            {
        ...
        public T FindInputSlot<T>(int slotId) where T : GeometrySlot
        {
            foreach (var slot in m_Slots.SelectValue())
            {
        ...
        public T FindOutputSlot<T>(int slotId) where T : GeometrySlot
        {
            foreach (var slot in m_Slots.SelectValue())
            {
        ...
        public virtual IEnumerable<GeometrySlot> GetInputsWithNoConnection()
        {
            return this.GetInputSlots<GeometrySlot>().Where(x => !owner.GetEdges(GetSlotReference(x.id)).Any());
        }
