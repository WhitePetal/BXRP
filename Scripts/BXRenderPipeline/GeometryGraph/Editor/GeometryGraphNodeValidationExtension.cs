...
namespace BXGeometryGraph
{
    internal interface INodeValidationExtension
    {
        ...
        Status GetValidationStatus(AbstractGeometryNode node, out string msg);
    }
    ...
    internal static class NodeValidation
    {
        ...
        {
            s_validators = new List<INodeValidationExtension>();
        ...
        internal static void HandleValidationExtensions(AbstractGeometryNode node)
        {
            ...
            foreach(var validator in s_validators)
            {
                ...
                {
                    var severity = status == INodeValidationExtension.Status.Warning ? GeometryCompilerMessageSeverity.Warning : GeometryCompilerMessageSeverity.Error;
                    node.owner.messageManager.AddOrAppendError(validator, node.objectId, new GeometryMessage(msg, severity));
                }
            }
        }
    }
