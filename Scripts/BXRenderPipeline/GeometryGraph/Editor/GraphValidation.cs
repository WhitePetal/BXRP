...
namespace BXGeometryGraph
{
    sealed partial class GraphData : ISerializationCallbackReceiver
    {
        ...
        {
            private static Dictionary<Type, FieldInfo> s_ActiveSubTarget = new();
            private static readonly PropertyInfo s_JsonData = typeof(JsonData<SubTarget>).GetProperty("value");

            private static SubTarget GetActiveSubTarget(Target target)
            {
                // All targets have an active subtarget but it's not accessible from GeometryGraph
            ...
            public static void ValidateNode(AbstractGeometryNode node)
            {
                Type t = node.GetType();
                node.ValidateNode();
                if (!(node is BlockNode))
                {
                    ...
                    {
                        node.SetOverrideActiveState(AbstractGeometryNode.ActiveState.Implicit);
                    }
                }
            }
