...
{
    public abstract class GeometryInput : JsonObject
    {
        [SerializeField]
        ...
        private HashSet<IGeometryInputObserver> m_InputObservers = new();
        internal HashSet<IGeometryInputObserver> InputObservers => m_InputObservers;
        ...
        public string displayName
        {
            get
            {
                if (string.IsNullOrEmpty(m_Name))
                    return $"{concreteGeometryValueType}_{objectId}";
                return m_Name;
            }
            set
            {
        ...
        internal void AddObserver(IGeometryInputObserver observer)
        {
            m_InputObservers.Add(observer);
        }

        internal void RemoveObserver(IGeometryInputObserver observer)
        {
        ...
        private const int k_LatestDefaultRefNameVersion = 1;

        // this is used to know whether this geometry input is using:
        ...
        {
            // if serialization doesn't write to m_DefaultRefNameVersion, then it is an old geometry input, and should use the old default naming scheme
        ...
        // returns true if this geometry input is CURRENTLY using the new default reference name
        ...
        internal abstract ConcreteSlotValueType concreteGeometryValueType { get; }
        ...
        [SerializeField]
        private bool m_UseCustomSlotLabel = false;

        [SerializeField]
        private string m_CustomSlotLabel;
        ...
        internal abstract GeometryInput Copy();
