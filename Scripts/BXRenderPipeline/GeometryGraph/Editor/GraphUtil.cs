    ...
    internal static class GraphUtil
    {
        ...
        public static void CreateNewGraph()
        {
            ...
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0, graphItem,
                string.Format("New Geometry Graph.{0}", GeometryGraphImporter.Extension), null, null);
        }
        ...
        static void Visit(List<AbstractGeometryNode> outputList, Dictionary<string, AbstractGeometryNode> unmarkedNodes, AbstractGeometryNode node)
        {
            if (!unmarkedNodes.ContainsKey(node.objectId))
                return;
            foreach (var slot in node.GetInputSlots<GeometrySlot>())
            {
        ...
        //
        public static List<NodeType> FindDownStreamNodesOfType<NodeType>(AbstractGeometryNode node) where NodeType : AbstractGeometryNode
        {
            // Should never be called without a node
            Debug.Assert(node != null);

            HashSet<AbstractGeometryNode> visitedNodes = new HashSet<AbstractGeometryNode>();
            List<NodeType> vtNodes = new List<NodeType>();
            Queue<AbstractGeometryNode> nodeStack = new Queue<AbstractGeometryNode>();
            nodeStack.Enqueue(node);
            visitedNodes.Add(node);

            while (nodeStack.Count > 0)
            {
                AbstractGeometryNode visit = nodeStack.Dequeue();

                // Flood fill through all the nodes
                foreach (var slot in visit.GetInputSlots<GeometrySlot>())
                {
