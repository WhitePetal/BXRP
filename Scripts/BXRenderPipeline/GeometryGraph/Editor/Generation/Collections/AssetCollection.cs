...
{
    // this class is used to track asset dependencies in geometrygraphs and subgraphs
    // that is: it tracks files that the geometrygraph or subgraph must access to generate the geometry
    // this data is used to automatically re-import the geometrygraphs or subgraphs when any of the tracked files change
    [GenerationAPI]
    internal class AssetCollection
    {
        ...
        public enum Flags
        {
            SourceDependency = 1 << 0,     // GeometryGraph directly reads the source file in the Assets directory
            ArtifactDependency = 1 << 1,     // GeometryGraph reads the import result artifact (i.e. subgraph import)
