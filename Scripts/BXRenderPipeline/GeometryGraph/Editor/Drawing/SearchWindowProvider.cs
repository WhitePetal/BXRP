...
namespace BXGeometryGraph
{
	...
	class SearchWindowProvider : IDisposable
	{
		...
		GeometryStageCapability m_ConnectedSlotCapability; // calculated in GenerateNodeEntries
		...
		List<int> m_Ids;
		List<GeometrySlot> m_Slots = new List<GeometrySlot>();
        ...
        public void GenerateNodeEntries()
        {
            ...
            {
                m_ConnectedSlotCapability = GeometryStageCapability.All;
            }
            ...
            if (target is ContextView contextView)
            {
                ...
                if (contextView.contextData.geometryStage == GeometryStage.Vertex && !hideCustomInterpolators)
                {
            ...
            foreach (var type in NodeClassCache.knownNodeTypes)
            {
                ...
                {
                    var node = (AbstractGeometryNode)Activator.CreateInstance(type);
                    if (!node.ExposeToSearcher)
                        continue;

                    if (GeometryGraphPreferences.allowDeprecatedBehaviors && node.latestVersion > 0)
                    {
                        ...
                        {
                            var depNode = (AbstractGeometryNode)Activator.CreateInstance(type);
            ...
            if (!hideCustomInterpolators)
            {
                ...
                //}
                foreach (var cibnode in m_Graph.geometryContext.blocks.Where(b => b.value.isCustomBlock))
                {
                    //var node = Activator.CreateInstance<CustomInterpolatorNode>();
                    //node.ConnectToCustomBlock(cibnode.value);
                    //AddEntries(node, new[] { "Custom Interpolator", cibnode.value.customName }, nodeEntries);
        ...
        void AddEntries(AbstractGeometryNode node, string[] title, List<NodeEntry> addNodeEntries)
        {
