...
namespace BXGeometryGraph
{
	public class GraphDropTarget : Manipulator
	{
		...
		private List<ISelectable> m_Selection;

		public GraphDropTarget(AbstructGeometryGraph graph)
		{
			m_Graph = graph;
		}
		...
		{
			m_GraphView = target as GeometryGraphView;
		...
		private bool ValidateObject(Object obj)
		{
			return EditorUtility.IsPersistent(obj)
				&& (obj is Texture2D || obj is Cubemap /** || obj is GeometrySubGraphAsset **/);
		}
		...
		private void CreateNode(object obj, Vector2 nodePosition)
		{
			...
			if(texture2D != null)
			{
				...
				{
					var impoerter = AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture2D)) as TextureImporter;
					if (impoerter != null)
						isNormalMap = impoerter.textureType == TextureImporterType.NormalMap;
				}
				...
				//drawState.position = new Rect(nodePostion, drawState.position.size);
				//node.drawState = drawState;
				//m_Graph.AddNode(node);
				//var inputslot = node.FindSlot<Texture2DInputGeometrySlot>(SampleTexture2DNode.TextureInputId);
				//if (inputslot != null)
			...
			{
				var property = blackboardField.userData as IGeometryProperty;
				if (property != null)
				{
		...
		private void OnIMGUIEvent(IMGUIEvent evt)
		{
			if(evt.imguiEvent.type == EventType.DragUpdated || evt.imguiEvent.type == EventType.DragPerform)
			{
				try
				{
					...
					if (!(pickElement is GeometryGraphView))
						return;
