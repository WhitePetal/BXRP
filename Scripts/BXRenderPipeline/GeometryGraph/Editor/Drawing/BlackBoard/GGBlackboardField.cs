...
{
    class GGBlackboardField : GraphElement, IInspectable, IGGControlledElement<GeometryInputViewController>, IDisposable
    {
        static readonly Texture2D k_ExposedIcon = Resources.Load<Texture2D>("GraphView/Nodes/BlackboardFieldExposed");
        static readonly string k_UxmlTemplatePath = "UXML/Blackboard/GGBlackboardField";
        static readonly string k_StyleSheetPath = "Styles/GGBlackboard";

        GeometryInputViewModel m_ViewModel;

        GeometryInputViewModel ViewModel
        {
            get => m_ViewModel;
        ...
        internal GGBlackboardField(GeometryInputViewModel viewModel)
        {
            ViewModel = viewModel;
            ...
            if (userData == null)
            {
                throw new AssertionException("Could not initialize blackboard field as geometry input was null.", null);
            }
            ...
            GeometryGraphPreferences.onAllowDeprecatedChanged += UpdateTypeText;
            ...
            var blackboard = ViewModel.parentView.GetFirstAncestorOfType<GGBlackboard>();
            if (blackboard != null)
            {
        ...
        void AddContextMenuOptions(ContextualMenuPopulateEvent evt)
        {
            ...
            if (geometryInput.isRenamable &&
                !string.IsNullOrEmpty(geometryInput.overrideReferenceName))
            {
                evt.menu.AppendAction(
                    ...
                    e =>
                    {
                        ...
                        resetReferenceNameAction.geometryInputReference = geometryInput;
            ...
            //if (geometryInput is ColorGeometryProperty colorProp)
            //{
            //    PropertyNodeView.AddMainColorMenuOptions(evt, colorProp, controller.graphData, m_InspectorUpdateDelegate);
            //}


            //if (geometryInput is Texture2DGeometryProperty texProp)
            //{
        ...
        GeometryInputViewController m_Controller;
        ...
        public GeometryInputViewController controller
        {
            get => m_Controller;
            set
            {
        ...
        // --- IGGControlledElement implementation

        [Inspectable("Geometry Input", null)]
        public GeometryInput geometryInput => ViewModel.model;
        ...
        void OnMouseHover(EventBase evt, GeometryInput input)
        {
            ...
            if (evt.eventTypeId == MouseEnterEvent.TypeId())
            {
                ...
                {
                    if (input is AbstractGeometryProperty property)
                    {
                    ...
                    else if (input is GeometryDropdown dropdown)
                    {
        ...
        void UpdateTypeText()
        {
            if (geometryInput is AbstractGeometryProperty asp)
            {
