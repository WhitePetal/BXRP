...
namespace BXGeometryGraph
{
    ...
    class GGBlackboard : GraphSubWindow, IGGControlledElement<BlackboardController>
    {
        ...
        IList<GGBlackboardCategory> m_BlackboardCategories = new List<GGBlackboardCategory>();
        ...
        public override string elementName => "GGBlackboard";
        public override string styleName => "GGBlackboard";
        public override string UxmlName => "Blackboard/GGBlackboard";
        public override string layoutKey => "BXGeometryGraph.Blackboard";
        ...
        public GGBlackboard(BlackboardViewModel viewModel, BlackboardController controller) : base(viewModel)
        {
            ViewModel = viewModel;
            ...
            if (ParentView is GeometryGraphView materialGraphView)
        ...
        public void OnDragEnterEvent(DragEnterEvent evt)
        {
            if (!m_IsUserDraggingItems)
            {
                ...
                SetCategoryDragIndicatorVisible(selection.OfType<GGBlackboardCategory>().Any());
            }
        }
        ...
        {
            if (m_IsUserDraggingItems && selection.OfType<GGBlackboardCategory>().Any())
        ...
        void OnDragUpdatedEvent(DragUpdatedEvent evt)
        {
            ...
            foreach (ISelectable selectedElement in selection)
            {
                ...
                if (sourceItem is GGBlackboardCategory blackboardCategory && blackboardCategory.controller.Model.IsNamedCategory() == false)
                {
        ...
        void OnDragPerformEvent(DragPerformEvent evt)
        {
            ...
            {
                if (item is GGBlackboardCategory category)
                {
                    ...
                    if (doesCategoryExistInGraph == false)
                    {
                        ...
                        {
                            if (otherItem is GGBlackboardField blackboardField && category.Contains(blackboardField))
            ...
            foreach (var item in selection.ToList())
            {
                if (item is GGBlackboardField blackboardField)
                {
                    ...
                    {
                        var copyGeometryInputAction = new CopyGeometryInputAction();
                        copyGeometryInputAction.geometryInputToCopy = selectedBlackboardItem;
                        ViewModel.requestModelChangeAction(copyGeometryInputAction);
                        selection.Remove(item);
                    }
                }
            }
            ...
            moveCategoryAction.categoryGuids = selection.OfType<GGBlackboardCategory>().OrderBy(sgcat => sgcat.GetPosition().y).Select(cat => cat.viewModel.associatedCategoryGuid).ToList();
        ...
        void InitializeAddBlackboardItemMenu()
        {
            ...
            if (ViewModel == null)
            {
                throw new AssertionException("SGBlackboard: View Model is null.", null);
            }
            ...
            foreach (var nameToAddActionTuple in ViewModel.propertyNameToAddActionMap)
            {
                ...
                AddGeometryInputAction addAction = nameToAddActionTuple.Value as AddGeometryInputAction;
            ...
            if (ViewModel.defaultDropdownNameToAdd != null)
            {
                ...
                AddGeometryInputAction addAction = ViewModel.defaultDropdownNameToAdd.Item2 as AddGeometryInputAction;
                addAction.categoryToAddItemToGuid = selectedCategoryGuid;
