...
{
    sealed class GGBlackboardCategory : GraphElement, IGGControlledElement<BlackboardCategoryController>, ISelection, IComparable<GGBlackboardCategory>
    {
        ...
        GGController IGGControlledElement.controller => m_Controller;
        ...
        const string k_StylePath = "Styles/GGBlackboard";
        const string k_UxmlPath = "UXML/Blackboard/GGBlackboardCategory";
        ...
        int m_InsertIndex;
        GGBlackboard blackboard => m_ViewModel.parentView as GGBlackboard;
        ...
        int InsertionIndex(Vector2 pos)
        {
            ...
            var blackboardRows = this.Query<GGBlackboardRow>().ToList();
        ...
        static VisualElement FindCategoryDirectChild(GGBlackboardCategory blackboardCategory, VisualElement element)
        {
        ...
        internal GGBlackboardCategory(BlackboardCategoryViewModel categoryViewModel, BlackboardCategoryController inController)
        {
        ...
        void OnFoldoutToggle(ChangeEvent<bool> evt)
        {
            if (evt.previousValue != evt.newValue)
            {
                ...
                if (selection.Contains(this)) // expand all selected if the foldout is part of a selection
                    isExpandedAction.categoryGuids = selection.OfType<GGBlackboardCategory>().Select(s => s.viewModel.associatedCategoryGuid).ToList();
                else
        ...
        void OnMouseDownEvent(MouseDownEvent e)
        {
            ...
            else if (e.clickCount == 1 && e.button == (int)MouseButton.LeftMouse && IsRenamable())
            {
                ...
                var fieldViews = this.Query<GGBlackboardField>();
        ...
        void OnDragUpdatedEvent(DragUpdatedEvent evt)
        {
            ...
            if (selection.OfType<GGBlackboardCategory>().Any())
            {
            ...
            if (selection.OfType<GGBlackboardField>().Any() == false)
            {
                SetDragIndicatorVisible(false);
                return;
            }
        ...
        void OnDragPerformEvent(DragPerformEvent evt)
        {
            ...
            SortedDictionary<GGBlackboardCategory, List<VisualElement>> draggedElements = new SortedDictionary<GGBlackboardCategory, List<VisualElement>>();
            ...
            foreach (ISelectable selectedElement in selection)
            {
                ...
                {
                    var otherCategory = draggedElement.GetFirstAncestorOfType<GGBlackboardCategory>();
            ...
            foreach (var categoryToChildrenTuple in draggedElements)
            {
                ...
                {
                    var blackboardField = draggedElement.Q<GGBlackboardField>();
                    GeometryInput shaderInput = null;
                    ...
                    var moveGeometryInputAction = new MoveGeometryInputAction();
                    moveGeometryInputAction.associatedCategoryGuid = viewModel.associatedCategoryGuid;
                    moveGeometryInputAction.geometryInputReference = shaderInput;
                    moveGeometryInputAction.newIndexValue = insertIndex;
                    m_ViewModel.requestModelChangeAction(moveGeometryInputAction);
        ...
        public void AddToSelection(ISelectable selectable)
        {
            ...
            var geometryGraphView = m_ViewModel.parentView.GetFirstAncestorOfType<GeometryGraphView>();
            geometryGraphView?.AddToSelection(selectable);
        }

        public void RemoveFromSelection(ISelectable selectable)
        {
            var materialGraphView = m_ViewModel.parentView.GetFirstAncestorOfType<GeometryGraphView>();
        ...
        public void ClearSelection()
        {
            RemoveFromClassList("selected");
            var geometryGraphView = m_ViewModel.parentView.GetFirstAncestorOfType<GeometryGraphView>();
            geometryGraphView?.ClearSelection();
        }
        ...
        {
            var geometryGraphView = blackboard.ParentView as GeometryGraphView;
            geometryGraphView?.deleteSelection?.Invoke("Delete", GraphView.AskUser.DontAskUser);
        }
        ...
        public int CompareTo(GGBlackboardCategory other)
        {
            if (other == null)
                return 1;
