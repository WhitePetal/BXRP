...
namespace BXGeometryGraph
{
    ...
    sealed partial class GraphData : JsonObject
    {
        ...
        [SerializeField]
        private List<JsonData<StickyNoteData>> m_StickyNoteDatas = new List<JsonData<StickyNoteData>>();
        ...
        [NonSerialized]
        private List<PotentialTarget> m_AllPotentialTargets = new List<PotentialTarget>();
        ...
        public bool hasVFXCompatibleTarget => false;
        ...
        private void AddKnownTargetsToPotentialTargets()
        {
        ...
        public void AddNode(AbstractGeometryNode node)
        {
            if(node is AbstractGeometryNode geometryNode)
            {
                if(isSubGraph && !geometryNode.allowedInSubGraph)
                {
                    Debug.LogWarningFormat("Attempting to add {0} to Sub Graph. This is not allowed.", geometryNode.GetType());
                    return;
                }

                AddNodeNoValidate(geometryNode);
        ...
        private void RemoveGroupNoValidate(GroupData group)
        {
        ...
        private void RemoveNoteNoValidate(StickyNoteData stickyNote)
        {
            if (!m_StickyNoteDatas.Contains(stickyNote))
            {
        ...
        public void UpdateActiveBlocks(List<BlockFieldDescriptor> activeBlockDescripotrs)
        {
            ...
            foreach(var geometryBlock in geometryContext.blocks)
            {
                if (geometryBlock.value?.isCustomBlock == true)
                {
                    geometryBlock.value.SetOverrideActiveState(disableCI ? AbstractGeometryNode.ActiveState.ExplicitInactive : AbstractGeometryNode.ActiveState.ExplicitActive);
                }
                else if(geometryBlock.value?.descriptor?.isUnknown == true)
                {
                    geometryBlock.value.SetOverrideActiveState(AbstractGeometryNode.ActiveState.ExplicitInactive);
                }
                else
                {
                    geometryBlock.value.SetOverrideActiveState(activeBlockDescripotrs.Contains(geometryBlock.value.descriptor) ? AbstractGeometryNode.ActiveState.ExplicitActive
                        : AbstractGeometryNode.ActiveState.ExplicitInactive);
                }
            }
        }
        ...
        public void AddRemoveBlocksFromActiveList(List<BlockFieldDescriptor> activeBlockDescriptors)
        {
            ...
            {
                // custom interpolators are fine.
                for (int i = 0; i < contextData.blocks.Count; ++i)
                {
                    if (contextData.blocks[i].value?.isCustomBlock == true)
                        continue;
                    ...
                    if (!activeBlockDescriptors.Contains(block.value.descriptor))
                    {
                        ...
                        var slot = block.value.FindSlot<GeometrySlot>(0);
            ...
            {
                if (descriptor.geometryStage != contextData.geometryStage)
                    return;
            ...
            GetBlocksToRemoveForContext(geometryContext);
            ...
            {
                TryAddBlockToContext(descriptor, geometryContext);
            }
        }

        private void AddNodeNoValidate(AbstractGeometryNode node)
        {
        ...
        public void RemoveNode(AbstractGeometryNode node)
        {
        ...
        private void RemoveNodeNoValidate(AbstractGeometryNode node)
        {
        ...
        private IEdge ConnectNoValidate(SlotReference fromSlotRef, SlotReference toSlotRef, bool assumeBatchSafety = false)
        {
            ...
            var fromSlot = fromNode.FindSlot<GeometrySlot>(fromSlotRef.slotId);
            var toSlot = toNode.FindSlot<GeometrySlot>(toSlotRef.slotId);
        ...
        public void RemoveElements(AbstractGeometryNode[] nodes, IEdge[] edges, GroupData[] groups, StickyNoteData[] notes, GeometryInput[] inputs = null)
        {
            ...
            if(inputs != null)
            {
                foreach(var geometryInput in inputs)
                {
                    RemoveGraphInputNoValidate(geometryInput);
                }
            }
        ...
        private void RemoveEdgeNoValidate(IEdge e, bool reevalutaeActivity = true)
        {
            ...
            AbstractGeometryNode input = e.inputSlot.node, output = e.outputSlot.node;
            if(input != null && GeometryGraphPreferences.autoAddRemoveBlocks)
            {
        ...
        public AbstractGeometryNode GetNodeFromId(string nodeId)
        {
        ...
        //internal Texture2DGeometryProperty GetMainTexture()
        //{
        ...
        public bool ContainsInput(GeometryInput geometryInput)
        {
            if (geometryInput == null)
                return false;

            return properties.Contains(geometryInput) || keywords.Contains(geometryInput) || dropdowns.Contains(geometryInput);
        }

        public bool ContainsNode(AbstractGeometryNode node)
        {
            if (node == null)
                return false;
        ...
        public void GetEdges(GeometrySlot slot, List<IEdge> foundEdges)
        {
        ...
        public void GetEdges(AbstractGeometryNode node, List<IEdge> foundEdges)
        {
        ...
        public IEnumerable<IEdge> GetEdges(AbstractGeometryNode node)
        {
        ...
        public void CollectGeometryProperties(PropertyCollector collector, GenerationMode generationMode)
        {
            foreach(var prop in properties)
            {
                ...
                //if(prop is GradientGeometryProperty gradientProp && generationMode == GenerationMode.Preview)
                //{
        ...
        public bool IsInputAllowedInGraph(GeometryInput input)
        {
            return (isSubGraph && input.allowedInSubGraph) || (!isSubGraph && input.allowedInMainGraph);
        }

        public bool IsInputAllowedInGraph(AbstractGeometryNode node)
        {
        ...
        public void AddGraphInput(GeometryInput input, int index = -1)
        {
            if (input == null)
                return;
        ...
        internal void AddGraphInputNoSanitization(GeometryInput input, int index = -1)
        {
            if (input == null)
                return;
        ...
        // only ignores names matching ignoreName on properties matching ignoreGuid
        public List<string> BuildPropertyReferenceNameList(AbstractGeometryProperty ignoreProperty, string ignoreName)
        {
            List<String> result = new List<string>();
            foreach(var p in properties)
            {
        ...
        public void ReplaceWith(GraphData other)
        {
            ...
            if((this.geometryContext.position != other.geometryContext.position))
            {
                this.geometryContext.position = other.geometryContext.position;
                m_MovedContexts = true;
            }

            using(var inputsToRemove = PooledList<GeometryInput>.Get())
            {
            ...
            using (var nodesToRemove = PooledList<AbstractGeometryNode>.Get())
            {
            ...
            foreach (var node in other.GetNodes<AbstractGeometryNode>())
            {
                if (node is BlockNode blockNode)
                {
                    //var contextData = blockNode.descriptor.geometryStage == GeometryStage.Vertex ? vertexContext : fragmentContext;
                    var contextData = geometryContext;
        ...
        internal void PasteGraph(CopyPasteGraph graphToPaste, List<AbstractGeometryNode> remappedNodes, List<Edge> remappedEdges)
        {
            ...
            var nodeList = graphToPaste.GetNodes<AbstractGeometryNode>();
            foreach (var node in nodeList)
            {
                ...
                AbstractGeometryNode pastedNode = node;
        ...
        private static T DeserializeLegacy<T>(string typeString, string json, Guid? overrideObjectId = null) where T : JsonObject
        {
            ...
            // we need some created objects to have deterministic objectIDs, because they affect the generated geometry.
            // if the generated geometry is not deterministic, it can create ripple effects (i.e. causing Geometrys to be modified randomly as properties are renamed)
        ...
        private static AbstractGeometryNode DeserializeLegacyNode(string typeString, string json, Guid? overrideObjectId = null)
        {
            var jsonObj = MultiJsonInternal.CreateInstanceForDeserialization(typeString);
            var value = jsonObj as AbstractGeometryNode;
            if (value == null)
            {
                ...
                if (overrideObjectId.HasValue)
                    value.OverrideObjectId(overrideObjectId.Value.ToString("N"));
                MultiJsonInternal.Enqueue(value, json);
                return value as AbstractGeometryNode;
            }
            else
            {
                if (overrideObjectId.HasValue)
        ...
        public override void OnAfterDeserialize(string json)
        {
            ...
            //    {
            //        ggVersion = graphData0.m_Version;
            //    }
            //    else
            //    {
            ...
            //        var nodeGuidMap = new Dictionary<string, AbstractGeometryNode>();
            //        var propertyGuidMap = new Dictionary<string, AbstractGeometryProperty>();
            //        var keywordGuidMap = new Dictionary<string, ShaderKeyword>();
            //        var groupGuidMap = new Dictionary<string, GroupData>();

            //        var slotsField = typeof(AbstractGeometryNode).GetField("m_Slots", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            //        var defaultReferenceNameField = typeof(GeometryInput).GetField("m_DefaultReferenceName", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            //            var property = DserializeLegacy<AbstractGeometryNode>(serializedProperty.typeInfo.fullName, serializedProperty.JSONnodeData, propObjectId);
            //            if (property == null)
            //                continue;

            //            m_Properties.Add(property);

            //            var input0 = JsonUtility.FromJson<GeometryInput0>(serializedProperty.JSONnodeData);
            ...
            //            {
            //                // ColorGeometryProperty is the only Property case where `GetDefaultReferenceName` was overriden
            //                if (MultiJson.ParseType(serializedProperty.typeInfo.fullName) == typeof(ColorGeometryProperty))
            //                {
            //                    defaultReferenceNameField.SetValue(property, $"Color_{GuidEncoder.Encode(Guid.Parse(input0.m_Guid.m_GuidSerialized))}");
            //                }
            //                else
            //                {
            //                    defaultReferenceNameField.SetValue(property, $"{property.concreteGeometryValueType}_{GuidEncoder.Encode(Guid.Parse(input0.m_Guid.m_GuidSerialized))}");
            //                }
            //            }
            //        }
            ...
            //        {
            //            var node0 = JsonUtility.FromJson<AbstractGeometryNode0>(serializedNode.JSONNodeData);
            ...
            //            {
            //                propertyField.SetValue(node, (JsonRef<AbstractGeometryProperty>)property);
            //            }
            ...
            //            var slots = (List<JsonData<GeometrySlot>>)slotsField.GetValue(node);
            //            slots.Clear();
            ...
            //                var slot = DeserializeLegacy<GeometrySlot>(serializedSlot.typeInfo.fullName, serializedSlot.JSONnodeData, slotObjectId);
            //                if (slot == null)
            //                {
            //                    continue;
            //                }
            ...
            //        else
            //        {
            //            m_OutputNode = (AbstractGeometryNode)GetNodes<IGeometryNode1>().FirstOrDefault();
            //        }
        ...
        {
            // Deferred upgrades
            #region Deferred urgrades
            //if (ggVersion != latestVersion)
            //{
            //if (ggVersion < 2)
            //{
            //    var addedBlocks = ListPool<BlockFieldDescriptor>.Get();

            //    void UpgradeFromBlockMap(Dictionary<BlockFieldDescriptor> blockMap)
            //    {
            ...
            //    m_GepmetryContext.position = oldPosition;
            ...
            PooledList<(LegacyUnknownTypeNode, AbstractGeometryNode)> updatedNodes = PooledList<(LegacyUnknownTypeNode, AbstractGeometryNode)>.Get();
            foreach (var node in m_Nodes.SelectValue())
            {
                if (node is LegacyUnknownTypeNode lNode && lNode.foundType != null)
                {
                    AbstractGeometryNode legacyNode = (AbstractGeometryNode)Activator.CreateInstance(lNode.foundType);
            ...
            m_NodeDictionary = new Dictionary<string, AbstractGeometryNode>(m_Nodes.Count);
        ...
        private void ReplaceNodeWithNode(LegacyUnknownTypeNode nodeToReplace, AbstractGeometryNode nodeReplacement)
        {
            var oldSlots = new List<GeometrySlot>();
            nodeToReplace.GetSlots(oldSlots);
            var newSlots = new List<GeometrySlot>();
        ...
        {
            foreach (var node in GetNodes<AbstractGeometryNode>().OfType<IOnAssetEnabled>())
            {
                node.OnEnable();
            }

            GeometryGraphPreferences.onVariantLimitChanged += OnKeywordChanged;
        }
        ...
        public void OnDisable()
        {
            ...
            foreach (var node in GetNodes<AbstractGeometryNode>())
                node.Dispose();
        }
        ...
        internal void ValidateCustomBlockLimit()
        {
            ...
            foreach(var bnode in geometryContext.blocks.Where(jb => !jb.value.isCustomBlock).Select(b => b.value))
            {
            ...
            int errRange = GeometryGraphProjectSettings.instance.customInterpolatorErrorThreshold;
            int warnRange = GeometryGraphProjectSettings.instance.customInterpolatorWarningThreshold;
        ...
        private void ValidateContextBlocks()
        {
            void ValidateContext(ContextData contextData, GeometryStage expectedGeometryStage)
            {
                if (contextData == null)
                    return;
                ...
                {
                    var slots = block.value.GetInputSlots<GeometrySlot>();
                    foreach (var slot in slots)
                        FindAndReportSlotErrors(slot, expectedGeometryStage);
                }
            };

            //ValidateContext(vertexContext, ShaderStage.Vertex);
            //ValidateContext(fragmentContext, ShaderStage.Fragment);
            ValidateContext(geometryContext, GeometryStage.Geometry);
        }

        private void FindAndReportSlotErrors(GeometrySlot initialSlot, GeometryStage expectedGeometryStage)
        {
            var expectedCapability = expectedGeometryStage.GetGeometryStageCapability();
            var errorSourceSlots = new HashSet<GeometrySlot>();
            var visitedNodes = new HashSet<AbstractGeometryNode>();

            var graph = initialSlot.owner.owner;
            var slotStack = new Stack<GeometrySlot>();
            slotStack.Clear();
            ...
            while (slotStack.Any())
            {
                ...
                if (slot.isInputSlot)
                {
                    foreach (var edge in graph.GetEdges(slot.slotReference))
                    {
                        ...
                        var outputSlot = node.FindOutputSlot<GeometrySlot>(edge.outputSlot.slotId);
                ...
                else
                {
                    ...
                    var ownerSlots = slot.owner.GetInputSlots<GeometrySlot>(slot);
                    foreach (var ownerSlot in ownerSlots)
                        slotStack.Push(ownerSlot);
                }
            }

            bool IsEntireNodeStageLocked(AbstractGeometryNode node, GeometryStageCapability expectedNodeCapability)
            {
            ...
            foreach(var errorSourceSlot in errorSourceSlots)
            {
                ...
                string actionToTake;
                if (errorSourceSlot.stageCapability != GeometryStageCapability.None)
                {
                ...
                var invalidStageName = expectedGeometryStage.ToString().ToLower();
                string message = $"{errorSource} is not compatible with {invalidStageName} block {initialSlot.RawDisplayName()}, {actionToTake}.";
                AddValidationError(errorNode.objectId, message, GeometryCompilerMessageSeverity.Error);
            }
        }
    }
