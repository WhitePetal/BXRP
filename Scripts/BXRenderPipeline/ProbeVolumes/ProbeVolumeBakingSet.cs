...
#pragma warning restore 618
		...
		private void ResolveSharedCellData(List<int> cellIndices, NativeArray<ProbeBrickIndex.Brick> bricksData, NativeArray<byte> cellSharedData, NativeArray<byte> cellSupportData)
        {
			...
			for(var i = 0; i < cellIndices.Count; ++i)
            {
				...
				int sharedDataCount = sharedValidityMaskChunkSize * shChunkCount;
				var sourceValidityNeightMaskData = cellSharedData.GetSubArray(sharedDataChunkOffset, sharedDataCount);
				sharedDataChunkOffset += sharedDataCount;
                ...
                {
					int skyOcclusionCount = sharedSkyOcclusionL0L1ChunkSize * shChunkCount;
					if (prv.skyOcclusion)
                    {
						var sourceSkyOcclusionDataL0L1 = cellSharedData.GetSubArray(sharedDataChunkOffset, skyOcclusionCount).Reinterpret<ushort>(1);
					...
					sharedDataChunkOffset += skyOcclusionCount;
					if (bakedSkyShadingDirection)
                    {
						int skyShadingDriectionCount = sharedSkyShadingDirectionIndicesChunkSize * shChunkCount;
						if (prv.skyOcclusion && prv.skyOcclusionShadingDirection)
                        {
							var sourceSkyShadingDirectionIndices = cellSharedData.GetSubArray(sharedDataChunkOffset, skyShadingDriectionCount);
						...
						sharedDataChunkOffset += skyShadingDriectionCount;
					}
                }

                if (hasSupportData)
                {
					int positionCount = supportPositionChunkSize * shChunkCount;
					var sourcePositions = cellSupportData.GetSubArray(supportDataChunkOffset, positionCount).Reinterpret<Vector3>(1);
					supportDataChunkOffset += positionCount;
					cellData.probePositions = m_UseStreamingAsset ? new NativeArray<Vector3>(sourcePositions, Allocator.Persistent) : sourcePositions;

					int validityCount = supportValidityChunkSize * shChunkCount;
					var sourceValidity = cellSupportData.GetSubArray(supportDataChunkOffset, validityCount).Reinterpret<float>(1);
					supportDataChunkOffset += validityCount;
					cellData.validity = m_UseStreamingAsset ? new NativeArray<float>(sourceValidity, Allocator.Persistent) : sourceValidity;

					int touchupCount = supportTouchupChunkSize * shChunkCount;
					var sourceTouchup = cellSupportData.GetSubArray(supportDataChunkOffset, touchupCount).Reinterpret<float>(1);
					supportDataChunkOffset += touchupCount;
                    ...
                    {
						int layerCount = supportLayerMaskChunkSize * shChunkCount;
						var sourceLayer = cellSupportData.GetSubArray(supportDataChunkOffset, layerCount).Reinterpret<byte>(1);
						supportDataChunkOffset += layerCount;
                    ...
                    {
						int offsetVectorCount = supportOffsetsChunkSize * shChunkCount;
						var sourceOffsetVectors = cellSupportData.GetSubArray(supportDataChunkOffset, offsetVectorCount).Reinterpret<Vector3>(1);
        ...
        {
			if (cellDescs.TryGetValue(cellIndex, out var cellData))
				return cellData;
			else
				return null;
        }
