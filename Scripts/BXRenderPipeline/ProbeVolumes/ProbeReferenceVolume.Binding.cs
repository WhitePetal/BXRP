...
{
    public partial class ProbeReferenceVolume : MonoBehaviour
    {
        ...
        /// <param name="isProbeVolumeEnable">True if APV is enabled</param>
        public void BindAPVRuntimeResources(CommandBuffer cmdBuffer, bool isProbeVolumeEnable)
        {
            bool needToBindNeutral = true;
            var refVolume = ProbeReferenceVolume.instance;

            // Do this only if probe volume is enable
            if(isProbeVolumeEnable && m_ProbeReferenceVolumeInit)
            {
                ...
                if (validResources)
                {
                    ...
                    cmdBuffer.SetGlobalTexture(ShaderIDs._APVProbeOcclusion, rr.ProbeOcclusion ?? (RenderTargetIdentifier)BXUtils.whiteVolumeTexture); // before unity 2023 version CoreUtils.whiteVolumeTexture not exists
            ...
            if (needToBindNeutral)
            {
                ...
                cmdBuffer.SetGlobalTexture(ShaderIDs._APVProbeOcclusion, (RenderTargetIdentifier)BXUtils.whiteVolumeTexture); // before unity 2023 version CoreUtils.whiteVolumeTexture not exists
            }
        }
        ...
        /// <param name="probeVolumesOptions">probe volume options from the active volume stack</param>
        ...
        public bool UpdateShaderVariablesProbeVolumes(CommandBuffer cmd, ProbeVolumesOptions probeVolumesOptions, int taaFrameIndex, bool supportRenderingLayers = false)
        {
            ...
            if (enableProbeVolumes)
            {
                ...
                parameters.normalBias = probeVolumesOptions.normalBias_runtime;
                parameters.viewBias = probeVolumesOptions.viewBias_runtime;
                parameters.scaleBiasByMinDistanceBetweenProbe = probeVolumesOptions.scaleBiasWithMinProbeDistance_runtime;
                parameters.samplingNoise = probeVolumesOptions.samplingNoise_runtime;
                parameters.weight = probeVolumesOptions.intensityMultiplier_runtime;
                parameters.leakReductionMode = probeVolumesOptions.leakReductionMode_runtime;
                parameters.frameIndexForNoise = taaFrameIndex * (probeVolumesOptions.animateSamplingNoise_runtime ? 1 : 0);
                parameters.reflNormalizationLowerClamp = 0.005f;
                parameters.reflNormalizationUpperClamp = probeVolumesOptions.occlusionOnlyReflectionNormalization_runtime ? 1f : 7f;
