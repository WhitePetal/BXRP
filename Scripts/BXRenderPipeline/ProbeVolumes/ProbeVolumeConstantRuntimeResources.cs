...
{
    using RuntimeResources = ProbeReferenceVolume.RuntimeResources;
    ...
    internal static class ProbeVolumeConstantRuntimeResources
    {
...
#if UNITY_EDITOR
        private static uint3 GetSampleOffset(uint i)
        {
            return new uint3(i, i >> 1, i >> 2) & 1;
        }
        private static int GetProbeIndex(int x, int y, int z)
        {
            return x + y * 2 + z * 4;
        }
        private static uint BuildFace(int axis, int idx)
        {
            uint mask = 0;
        ...
        private static bool TryGetEdge(uint validityMask, uint samplingMask, out uint edge, out uint3 offset)
        {
            for(int i = 0; i < 8; ++i)
            {
        ...
        private static List<uint3> ComputeMask(uint validityMask)
        {
        ...
        private static uint PackSamplingDir(uint val)
        {
            ...
            return /* 2 bits */ (val << 1) | /* 1 bits */ ((~val & 2) >> 1);
        }

        private static uint InvalidSampleMask()
        {
            // This is a special code that results in no sampling in shader without any additional ALU
            return 2 | (2 << 3) | (2 << 6);
        }

        private static uint ComputeAntiLeakData(uint validityMask)
        {
        ...
        private static uint[] BuildAntiLeakDataArray()
        {
