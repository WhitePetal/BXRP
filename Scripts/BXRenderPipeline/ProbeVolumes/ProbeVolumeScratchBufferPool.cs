...
namespace BXRenderPipeline
{
	...
	internal class ProbeVolumeScratchBufferPool
	{
		...
		public ProbeVolumeScratchBufferPool(ProbeVolumeBakingSet bakingSet, ProbeVolumeSHBands shBands)
		{
			...
			m_ValidityLayerSize = bakingSet.bakedMaskCount;
		...
		private CellStreamingScratchBufferLayout GetOrCreateScratchBufferLayout(int chunkCount)
		{
			...
			else
			{
				...
				bufferLayout._ValidityProbeSize = m_ValidityLayerSize; // 1layer => 1xbyte => 4 probes at a time.
				if(m_SkyOcclusionSize != 0)
				{
					...
					{
						bufferLayout._SkyShadingDirectionProbeSize = m_SkyShadingDirectionSize;
				...
				int destChunkSize = chunkCount * sizeof(uint) * 4;

				bufferLayout._SharedDestChunksOffset = destChunkSize;
				bufferLayout._L0L1rxOffset = bufferLayout._SharedDestChunksOffset + destChunkSize;
		...
		private CellStreamingScratchBuffer CreateScratchBuffer(int chunkCount, bool allocateGraphicsBuffer)
		{
			var scratchBuffer = new CellStreamingScratchBuffer(chunkCount, chunkSize, allocateGraphicsBuffer);
