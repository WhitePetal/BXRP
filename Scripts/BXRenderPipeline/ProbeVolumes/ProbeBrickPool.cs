...
{
	using CellStreamingScratchBuffer = ProbeReferenceVolume.CellStreamingScratchBuffer;
	using CellStreamingScratchBufferLayout = ProbeReferenceVolume.CellStreamingScratchBufferLayout;
	using Cell = ProbeReferenceVolume.Cell;
	...
	internal class ProbeBrickPool
	{
		...
		internal static int EstimateMemoryCostForBlending(ProbeVolumeTextureMemoryBudget memoryBudget, bool compressed, ProbeVolumeSHBands shBands)
		{
			if (memoryBudget == 0)
				return 0;
		...
		internal void Update(DataLocation source, List<BrickChunkAlloc> srcLocations, List<BrickChunkAlloc> dstLocations, int destStartIndex, ProbeVolumeSHBands shBands)
		{
		...
		internal void Update(CommandBuffer cmd, CellStreamingScratchBuffer dataBuffer, CellStreamingScratchBufferLayout layout, 
			List<BrickChunkAlloc> dstLocations, bool updateSharedData, Texture validityTexture, ProbeVolumeSHBands shBands, 
		...
		{
			using(new ProfilingScope(cmd, ProfilingSampler.Get(BXProfileId.APVDiskStreamingUpdatePool)))
			{
				...
				const int probePerThreads = 4; // We can upload 4 probes per thread in the current shader.
				int threadX = DivRoundUp(kChunkSizeInBricks * kBrickProbeCountTotal / probePerThreads, numThreads);
		...
		internal void UpdateValidity(DataLocation source, List<BrickChunkAlloc> srcLocations, List<BrickChunkAlloc> dstLocations, int destStartIndex)
		{
			...
			{
				BrickChunkAlloc src = srcLocations[i];
				BrickChunkAlloc dst = dstLocations[i + destStartIndex];

				for(int j = 0; j < kBrickProbeCountPerDim; ++j)
				{
					int width = Mathf.Min(kChunkSizeInBricks * kBrickProbeCountPerDim, source.width - src.x);
					Graphics.CopyTexture(source.TexValidity, src.z + j, 0, src.x, src.y, width, kBrickProbeCountTotal, m_Pool.TexValidity, dst.z + j, 0, dst.x, dst.y);
				}
			}
		}
	...
	internal class ProbeBrickBlendingPool
	{
		...
		internal static void Initialize()
		{
			if (SystemInfo.supportsComputeShaders)
			{
				if(BXRenderPipeline.TryGetRenderCommonSettings(out var settings))
				{
					stateBlendShader = settings.probeVolumeRuntimeResources?.probeVolumeBlendStateCS;
		...
		private ProbeBrickPool m_State0, m_State1;
		...
		internal int estimatedVMemCoast
		{
			get
			{
		...
		internal void AllocateResourcesIfNeeded()
		{
			...
			// w*h*d / (chunkBricks * brickProbePerDim * brickProbePerDim * brickProbePerDim) =>
			// bricks / chunkBricks => chunks
			int maxAvailableChunkCount = (GetPoolWidth() / ProbeBrickPool.kChunkProbeCountPerDim)
									   * (GetPoolHeight() / ProbeBrickPool.kBrickProbeCountPerDim)
									   * (GetPoolDepth() / ProbeBrickPool.kBrickProbeCountPerDim);

			m_ChunkList = new Vector4[maxAvailableChunkCount];
			m_MappedChunks = 0;
		}

		internal void Update(ProbeBrickPool.DataLocation source, List<ProbeBrickPool.BrickChunkAlloc> srcLocations, List<ProbeBrickPool.BrickChunkAlloc> dstLocations, int destStartIndex, ProbeVolumeSHBands shBands, int state)
		{
			(state == 0 ? m_State0 : m_State1).Update(source, srcLocations, dstLocations, destStartIndex, shBands);
		}

		internal void Update(CommandBuffer cmd, CellStreamingScratchBuffer dataBuffer, CellStreamingScratchBufferLayout layout,
			List<ProbeBrickPool.BrickChunkAlloc> dstLocations, ProbeVolumeSHBands shBands, int state, Texture validityTexture,
			bool skyOcclusion, Texture skyOcclusionTexture, bool skyShadingDirections, Texture skyShadingDirectionsTexture, bool probeOcclusion)
		{
			bool updateShared = state == 0 ? true : false;

			(state == 0 ? m_State0 : m_State1).Update(cmd, dataBuffer, layout, dstLocations, updateShared, validityTexture, shBands, 
				updateShared && skyOcclusion, skyOcclusionTexture, updateShared && skyShadingDirections, skyShadingDirectionsTexture, probeOcclusion);
		}
		...
		internal void Deallocate(List<ProbeBrickPool.BrickChunkAlloc> outAllocations)
		{
			if (outAllocations.Count == 0)
				return;

			m_State0.Deallocate(outAllocations);
		}
