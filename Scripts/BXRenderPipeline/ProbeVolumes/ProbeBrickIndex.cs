...
using CellIndexInfo = BXRenderPipeline.ProbeReferenceVolume.CellIndexInfo;
...
namespace BXRenderPipeline
{
	internal class ProbeBrickIndex
	{
		...
		private Vector3Int m_CenterRS; // the anchor in ref space, around which the index is defined. [IMPORTANT NODE! For now we always have it at 0, so is not passed to the shader, but is kept here until development is active in case we find it useful]
		...
		internal ProbeBrickIndex(ProbeVolumeTextureMemoryBudget memoryBudget)
		{
			...
			m_AvaliableChunkCount = m_ChunksCount;
		...
		internal void ComputeFragmentationRate()
		{
			...
			int holes = m_AvaliableChunkCount - tailFreeChunks;
		...
		public struct IndirectionEntryUpdateInfo
		{
			...
			// IMPORTANT, These values shuld be at max resolution, independent of minSubdivInCell. This means that
		...
		internal bool FindSlotsForEntries(ref IndirectionEntryUpdateInfo[] entriesInfo)
		{
			...
			// The copy here is temporary and while it is created upon loading all the time, it is going to occupy memory
			// only when
			// a load operation happens
		...
		internal bool ReserveChunks(IndirectionEntryUpdateInfo[] entriesInfo, bool ignoreErrorLog)
		{
			...
			for(int entry = 0; entry < entryCount; ++entry)
			{
				...
				m_AvaliableChunkCount -= numberOfChunkForEntry;
			}
		...
		private void MarkBrickInPhysicalBuffer(in IndirectionEntryUpdateInfo entry, Vector3Int brickMin, Vector3Int brickMax, int brickSubdivLevel, int entrySubdivLevel, int idx)
		{
			...
			else
			{
				...
				else
				{
					...
					// Before finding the locatgions inside the Index for the current cell/chunk.
					...
					Debug.Assert(brickMin.x >= 0 && brickMin.y >= 0 && brickMin.z >= 0, "Brick in out of bounds");
				...
				// Analytically compute min and max because doing it in the inner loop with Math.Min/Max is costly (not inline)
