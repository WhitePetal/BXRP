#ifndef UNITY_SPHERICAL_HARMONICS_INCLUDE
#define UNITY_SPHERICAL_HARMONICS_INCLUDE
...
// Clamped cosine convolution coef (pre-divided by Pi)
...
half3 SampleSH9(half4 SHCoefficients[7], half3 N)
{
    ...
    // Linear + costant polynomial terms
...
half3 SampleSH9(StructuredBuffer<float4> data, half3 N)
{
...
// See SetSHMapConstants() in "Stupid Spherical Harmonics Tricks"
// Constan + linear
void PackSH(RWStructuredBuffer<float4> buffer, float sh[27])
{
    ...
    for(c = 0; c < 3; c++)
    {
        buffer[3 + c] = float4(sh[c * 9 + 4], sh[c * 9 + 5], sh[c * 9 + 2], sh[c * 9 + 0] - sh[c * 9 + 6]);
    }
