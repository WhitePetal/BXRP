...
uint GetIndexData(APVResources apvRes, float3 posWS)
{
    ...
    UNITY_BRANCH if (isALoadedCell)
    {
        ...
        if(LoadCellIndexMetaData(flatIdx, chunkIdx, stepSize, minRelativeIdx, sizeOfValid))
        {
            ...
            localBrickIndex -= minRelativeIdx; // Relatie to valid region
...
bool TryToGetPoolUVWAndSubdiv(APVResources apvRes, float3 posWSFroSample, out float3 uvw, out uint subdiv)
{
    // resolve the index
    uint packed_pool_idx = GetIndexData(apvRes, posWSFroSample.xyz);

    // unpack pool idx
    // size if encoded in the upper 4 bits
    ...
    float3 offset = frac(posWSFroSample.xyz / brickSizeWS); // [0;1] in brick space
    ...
    // Note: we could instead early return when we know we'll have invalid UVs, but some bade code gen on Vulkan generates shader warnings if wed
...
APVSample SampleAPV(APVResources apvRes, float3 uvw)
{
    ...
    apvSample.L1_B = L1B_L1Rz.xyz;
        ...
        apvSample.L2_C = half4(SAMPLE_TEXTURE3D_LOD(apvRes.L2_3, s_linear_clamp_sampler, uvw, 0).rgb);
    #endif
    ...
    #ifdef USE_APV_PROBE_OCCLUSION
        apvSample.probeOcclusion = SAMPLE_TEXTURE3D_LOAD(apvRes.ProbeOcclusion, s_linear_clamp_sampler, uvw, 0).rgba;
    #endif
...
float UnpackSamplingWeight(uint mask, float3 texFrace)
{
    int3 dir = 0;
    dir.x = (int)(mask >> 1) & 3;
    dir.y = (int)(mask >> 4) & 3;
    ...
    float3 weights;
    weights.x = saturate(mask & 1)  + (dir.x - 1) * texFrace.x;
    weights.y = saturate(mask & 8)  + (dir.y - 1) * texFrace.y;
    weights.z = saturate(mask & 64) + (dir.z - 1) * texFrace.z;
...
void WarpUVWLeakReduction(APVResources apvRes, uint renderingLayer, inout float3 uvw)
{
    ...
    if(validityMask != 0xFF)
    {
        ...
        for(i = 0; i < 8; ++i)
        {
            ...
            half validityWeight = 
                ((probeOffset.x == 1) ? texFrac.x : 1.0 - texFrac.x) *
                ((probeOffset.y == 1) ? texFrac.y : 1.0 - texFrac.y) *
                ((probeOffset.z == 1) ? texFrac.z : 1.0 - texFrac.z);
...
APVSample SampleAPV(APVResources apvRes, float3 posWS, float3 biasNormalWS, uint renderingLayer, float3 viewDir)
{
    ...
    float3 pool_uvw;
    float3 biasedPosWS;
    if(TryToGetPoolUVWAndSubdiv(apvRes, posWS, biasedPosWS, viewDir, pool_uvw, subdiv, biasedPosWS))
    {
...
float3 EvaluateOccludedSky(APVSample apvSample, float3 N)
{
    ...
    if(_APVSkyDirectionWeight > 0)
    {
        ...
        if(normalSquared < 0.2f)
        {
            shadingNormal = N;
        }
        else
        {
            shadingNormal = shadingNormal * rsqrt(normalSquared);
        }
    }
...
float GetReflectionProbeNormalizationFactor(float3 lightingInReflDir, float3 sampleDir, float4 reflProbeSHL0L1, float4 reflProbeSHL2_1, float reflProbeSHL2_2)
{
    ...
    return lerp(1.0f, clamp(SafeDiv(localNormalization, refProbeNormalization), _APVMinReflProbeNormalizationFactor, _APVMaxReflProbeNormalizationFactor), _APVWeight);
}
