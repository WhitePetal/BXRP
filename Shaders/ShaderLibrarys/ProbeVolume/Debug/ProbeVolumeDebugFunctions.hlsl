...
float4 TransfromPosition(float3 posOS)
{
    ...
    v2f vert(appdata v)
    {
        ...
        if(!ShouldCull(o))
        {
            float3 probePosition_WS = TransfromPosition(0.0f).xyz;
            ...
            {
                float4 wsPos = TransfromPosition(v.vertex.xyz * _ProbeSize);
    ...
    float4 frag(v2f i) : SV_Target
    {
        ...
        {
            return float4(CalculateDiffuseLighting(i) * _ReleateExpourse, 1.0f);
        }
        else if(_ShadingMode == DEBUGPROBESHADINGMODE_INVALIDATED_BY_ADJUSTMENT_VOLUMES)
        {
            float4 defaultCol = float4(CalculateDiffuseLighting(i) * _ReleateExpourse, 1.0f);
    ...
    v2f vert(appdata v)
    {
        ...
        {
            float4 wsPos = TransfromPosition(v.vertex.xyz * float3(_OffsetSize, _OffsetSize, 1.0f));
            o.vertex = mul(UNITY_MATRIX_VP, wsPos);
            o.normal = normalize(mul(v.normal, (float3x3)UNITY_MATRIX_M));
        }
    ...
    v2f vert(appdata v)
    {
        ...
        if(v.color.z)
        {
            ...
            {
                quadPosition = snappedProbePosition_WS + float3(0.0, 1.0, 0.0) * probeDistance;
