...
Shader "Skybox" {
    ...
    //              for(int i=0; i<int(kSamples); i++)
                    {
                        ...
                        float lightAngle = dot(_DirectionalLightDirections[0].xyz, samplePoint) / height;
                ...
                else
                {
                    ...
                    float lightAngle = dot(_DirectionalLightDirections[0].xyz, pos);
    ...
    //              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
                ...
                OUT.skyColor    = _Exposure * (cIn * getRayleighPhase(_DirectionalLightDirections[0].xyz, -eyeRay));
    
            #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
                // The sun should have a stable intensity in its course in the sky. Moreover it should match the highlight of a purely specular material.
                // This matching was done using the standard shader BRDF1 on the 5/31/2017
                // Finally we want the sun to be always bright even in LDR thus the normalization of the lightColor for low intensity.
                half lightColorIntensity = clamp(length(_DirectionalLightColors[0].xyz), 0.25, 1);
                #if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
                    OUT.sunColor    = kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * _DirectionalLightColors[0].xyz / lightColorIntensity;
                #else // SKYBOX_SUNDISK_HQ
                    OUT.sunColor    = kHDSundiskIntensityFactor * saturate(cOut) * _DirectionalLightColors[0].xyz / lightColorIntensity;
                #endif
            ...
            // if y < 0 [eyeRay.y > 0] - sky
                ...
                if(y < 0.0)
                {
                    col += IN.sunColor * calcSunAttenuation(_DirectionalLightDirections[0].xyz, -ray);
                }
            #endif
