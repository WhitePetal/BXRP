Shader "PostProcess"
{
    ...
    SubShader
    {
		...
		// 6 Blit for Preview Window Editor Only
		Pass
		{
			HLSLPROGRAM
			#pragma target 4.5
			...
			half4 frag (v2f i) : SV_Target
			{
				half4 input = _PostProcessInput.SampleLevel(sampler_PostProcessInput, i.uv, 0);
				input.rgb = ToneMapping_ACES_To_sRGB(input.rgb, half(1.0));
				return input;
			}
			ENDHLSL
		}
